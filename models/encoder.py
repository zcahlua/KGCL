...
class MPNEncoder(nn.Module):
    ...
    def __init__(self, atom_fdim: int, bond_fdim: int, hidden_size: int,
       ...
       """
        ...
        self.W_o = nn.Sequential(
            nn.Linear(self.atom_fdim + self.hidden_size, self.hidden_size), nn.SELU())
    ...
    def forward(self, graph_tensors: Tuple[torch.Tensor], mask: torch.Tensor) -> torch.FloatTensor:
        ...
        mask: torch.Tensor,
            Masks on nodes
        """
        f_atoms, f_bonds, f_fgs, atom_num, n_mols_tensor, a2b, b2a, b2revb, undirected_b2a = graph_tensors
        # Input
