...
class KGCL(nn.Module):
    ...
    def compute_edit_scores(self, prod_tensors: Tuple[torch.Tensor],
        ...
        atom_feats = F.selu(self.W_vv(prev_atom_hiddens) + self.W_vc(a_feats))
        ...
        return edit_scores, prev_atom_hiddens, prev_atom_scope, graph_vecs
    ...
    def forward(self, prod_seq_inputs: List[Tuple[torch.Tensor, List]]):
        ...
        for idx in range(max_seq_len):
            ...
            edit_scores, prev_atom_hiddens, prev_atom_scope, graph_outs = self.compute_edit_scores(
                prod_tensors, prod_scopes, prev_atom_hiddens, prev_atom_scope)
            seq_edit_scores.append(edit_scores)
            batch_graph_outs.append(graph_outs)

        return seq_edit_scores, batch_graph_outs
    ...
    def predict(self, prod_smi: str, rxn_class: int = None, max_steps: int = 9):
        ...
        while not done and steps <= max_steps:
            ...
            edit_logits, prev_atom_hiddens, prev_atom_scope, graph_outs = self.compute_edit_scores(
                prod_tensors, prod_scopes, prev_atom_hiddens, prev_atom_scope)
