...
def canonicalize_prod(p):
    ...
    if p_mol is None:
        raise ValueError(f"Invalid SMILES string after canonicalization: {p}")
...
def infer_correspondence(p):
    ...
    matches = list(canon_mol.GetSubstructMatches(orig_mol,maxMatches=1))  # 若不设置，高度对称的大分子搜索将超时
    ...
    if matches:
        ...
        print(f"No matches found for SMILES: {p}")
