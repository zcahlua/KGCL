...
from kgcl.config.device import resolve_cuda_device
...
def run_training(config):
    ...
    with functional_group_resource_context(resource_root=args.get('resource_root'), root_dir=paths.root):
        import sys
        import joblib
        from rdkit import RDLogger
        from torch.optim import Adam, lr_scheduler
        cuda = resolve_cuda_device(args.get('device', 'cuda'))
        device = str(cuda.torch_device)
        # Import only after resources are configured and CUDA is resolved.
        ...
        try:
            ...
            for epoch in range(args['epochs']):
                ...
                if valid_acc >= best_acc:
                    ...
                    print('---------------------------------------------------------')
                    print()
                    save_checkpoint(model, context.output_dir, epoch)
                    best_acc = valid_acc
        finally:
            csv_logger.close()
