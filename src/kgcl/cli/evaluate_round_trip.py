...
def main(argv=None):
    ...
    checkpoint = torch.load(os.path.join(exp_dir, args.checkpoint or 'epoch_132.pt'))
    ...
    beam_model = BeamSearch(model=model, step_beam_size=args.step_beam_size, beam_size=args.beam_size, use_rxn_class=args.use_rxn_class)
