...
def main(argv=None):
    ...
    checkpoint = torch.load(os.path.join(exp_dir, args.checkpoint or 'epoch_168.pt'))
    ...
    top_k = np.zeros(getattr(args, "beam_size", args.full_beam_size))
    edit_steps_cor = []
    counter = []
    stereo_rxn = []
    stereo_rxn_cor = []
    beam_model = BeamSearch(model=model, step_beam_size=args.step_beam_size,
                            beam_size=getattr(args, "beam_size", args.full_beam_size), use_rxn_class=args.use_rxn_class)
