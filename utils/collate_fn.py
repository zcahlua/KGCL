...
def get_batch_graphs(graph_batch: List[MolGraph], use_rxn_class: bool = False) -> Tuple[torch.Tensor, List[Tuple[int]]]:
    ...
    for mol_graph in graph_batch:
        ...
        atom_num.append(mol_graph.n_atoms)
    ...
    f_fgs = torch.FloatTensor(f_fgs)
    atom_num = torch.tensor(atom_num)
    n_mols = torch.tensor(n_mols)

    graph_tensors = (f_atoms, f_bonds, f_fgs, atom_num, n_mols, a2b, b2a, b2revb, undirected_b2a)
